...
class Review():
    ...
    def get_review(self, shop_id, request_type='proxy, cookie', last_chance=False):
        ...
        while all_pages == -1 or all_pages > 0:
            url = 'http://www.dianping.com/shop/' + str(shop_id) + '/review_all/p' + str(cur_pages) + '?queryType=sortType&queryVal=latest'
            # 访问p1会触发验证码，因此对第一页单独处理
            if cur_pages == 1:
                url = 'http://www.dianping.com/shop/' + str(shop_id) + '/review_all?queryType=sortType&queryVal=latest'
            print(url)
